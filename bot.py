...
# КОНСТАНТЫ
# ═══════════════════════════════════════════════════════════════
OPG_FACTIONS = ["Арзамасская ОПГ", "Батыревская ОПГ", "Лыткаринская ОПГ"]
GOV_FACTIONS = ["Правительство", "ФСБ", "УМВД", "ГИБДД", "ВЧ", "МЗ", "СМИ"]
...
# ═══════════════════════════════════════════════════════════════
# КОМАНДЫ: КИКНУТЬ ВСЕХ
# ═══════════════════════════════════════════════════════════════

class KickAllConfirm(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("❌ Только вызвавший команду может подтвердить.", ephemeral=True)
            return False
        return True
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            embed=make_embed(discord.Color.orange(), "⏳ Выполняется...", "Кикаю участников, подождите."),
            view=self
        )
        ...
        for member in list(guild.members):
            ...
            if member.bot:
                skipped += 1
                continue
            # Нельзя кикнуть тех, кто выше или равен боту в иерархии
            if member.top_role >= guild.me.top_role:
                failed += 1
                continue
            try:
                await member.kick(reason=f"Массовый кик — {interaction.user}")
                kicked += 1
            except (discord.Forbidden, discord.HTTPException):
                failed += 1

        result_lines = [f"**Кикнуто:** {kicked}"]
        if failed:
            result_lines.append(f"**Не удалось:** {failed} (выше бота в иерархии или нет прав)")
        if skipped:
            result_lines.append(f"**Пропущено:** {skipped} (боты, вы)")
        result_lines.append(f"\n**Выполнил:** {interaction.user.mention}")

        await interaction.edit_original_response(
            embed=make_embed(discord.Color.red(), "🔨 Массовый кик завершён", "\n".join(result_lines)),
            view=self
        )
        self.stop()
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(
            embed=make_embed(discord.Color.greyple(), "❌ Отменено", "Массовый кик отменён."),
            view=self
        )
        self.stop()
...
@bot.tree.command(name="кикнуть-всех", description="Кикнуть всех участников сервера (кроме ботов и вас)")
@app_commands.checks.has_permissions(administrator=True)
async def kick_all_cmd(interaction: discord.Interaction):
    count = sum(1 for m in interaction.guild.members if not m.bot and m.id != bot.user.id and m.id != interaction.user.id)
    view = KickAllConfirm(author_id=interaction.user.id)
    await interaction.response.send_message(
        embed=make_embed(
            discord.Color.yellow(), "⚠️ Подтверждение",
            f"Вы собираетесь кикнуть **{count}** участников.\n\nЭто действие **необратимо**. Подтвердите или отмените."
        ),
        view=view,
        ephemeral=True
    )
